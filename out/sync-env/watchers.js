...
function watchFileCreate(file) {
    const fileName = _1.getFileName(file.path);
    if (_1.configMapper[fileName] && fs.existsSync(_1.getFilePath(file.path) + _1.configMapper[fileName])) {
        const targetFile = _1.readfile(`${_1.getFilePath(file.path)}${_1.configMapper[fileName]}`);
        vscode.window.showInformationMessage(`
          You just created an env file which you are 
          watching for changes. Do you want to copy 
          the content of the child(${_1.configMapper[fileName]}) to it?`, ...['No', 'Yes'])
            .then(response => {
            if (response === 'Yes') {
